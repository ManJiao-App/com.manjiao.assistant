...
internal class GlassLensRenderer {
    ...
    // backdrop 录制缓存判断字段
    private var lastOffsetX = Float.MIN_VALUE
    private var lastOffsetY = Float.MIN_VALUE
    private var lastBackdropParent: View? = null
    private var lastSvFrameAt = 0L
    ...
    fun draw(
        ...
        // backdrop 录制缓存：玻璃没动 + 视频帧没更新 → 复用上次 DisplayList。
        // 宿主（快手）decor 上任何 invalidate 都会连带触发本玻璃 draw，若每次都
        // 全树重录 backdrop，主线程直接被打满（菜单卡死点不动）。重录频率由
        // 视频帧驱动 = 8fps 封顶；玻璃触摸动画期间位置每帧变，照常重录。
        val reRecord = offsetX != lastOffsetX || offsetY != lastOffsetY ||
            width != lastWidth || height != lastHeight || margin != lastMargin ||
            parent !== lastBackdropParent || svCacheAt != lastSvFrameAt || renderNode.hasDisplayList().not()
        if (reRecord) {
            ...
            try {
                ...
                // SurfaceView（视频）内容在独立 layer，View.draw 画不出来 → 采样区全黑。
                // 异步 PixelCopy：只拷玻璃投影区（小图），回调线程更新缓存，
                // 主线程零阻塞（同步等待版每帧卡 50-80ms 是卡顿根源）
                drawSurfaceViews(recordingCanvas, parent, glassView, margin - offsetX, margin - offsetY, margin)
            } finally {
                renderNode.endRecording()
            }
            lastOffsetX = offsetX
            lastOffsetY = offsetY
            lastBackdropParent = parent
            lastSvFrameAt = svCacheAt
        }
    ...
    /**
     * 把 backdrop 树里的 SurfaceView（视频）内容补画进录制画布。
     * SurfaceView 内容在独立 compositor layer 上，View.draw 只画挖洞背景 → 采样区全黑。
     *
     * 性能方案（同步等待版每帧阻塞主线程 50-80ms 是卡顿根源）：
     * - 只拷「玻璃 ± margin 在 SurfaceView 上的投影区」——小图，且玻璃里显示的
     *   就是它正后方的画面（与背景融为一体），不是缩小的全屏画面
     * - 异步 PixelCopy：本帧直接用上一帧缓存（若有），回调线程成功后更新缓存并
     *   触发玻璃重绘，主线程零等待
     */
    private var svCacheBmp: Bitmap? = null
    private var svCacheAt = 0L
    private var svCacheW = 0
    private var svCacheH = 0
    @Volatile private var svCopyInFlight = false
    @Volatile private var svLastRequestAt = 0L

    // 刷新频率上限：8fps（125ms 一帧视频背景）。模糊玻璃后无感知差别，
    // 每次刷新 = 全树 backdrop 录制 + RenderEffect，频率高了主线程会被打满
    private val COPY_INTERVAL_MS = 125L

    private fun drawSurfaceViews(
        canvas: Canvas,
        parent: View,
        glassView: View,
        baseTx: Float,
        ...
        val surfaces = ArrayList<android.view.SurfaceView>(2)
        collectSurfaceViews(parent, glassView, surfaces)
        val sv = surfaces.firstOrNull {
            it.visibility == View.VISIBLE && it.width > 0 && it.height > 0
        } ?: return
        try {
            val holder = sv.holder ?: return
            ...
            glassView.getLocationOnScreen(gLoc)
            sv.getLocationOnScreen(sLoc)
            parent.getLocationOnScreen(hostLoc)

            val gw = glassView.width
            val gh = glassView.height
            if (gw <= 0 || gh <= 0) return

            // 玻璃 ± margin 在 SurfaceView 坐标系上的投影，clamp 到 surface 范围
            val l = (gLoc[0] - sLoc[0] - margin).coerceIn(0, sv.width)
            val t = (gLoc[1] - sLoc[1] - margin).coerceIn(0, sv.height)
            val r = (gLoc[0] - sLoc[0] + gw + margin).coerceIn(0, sv.width)
            val b = (gLoc[1] - sLoc[1] + gh + margin).coerceIn(0, sv.height)
            if (r - l <= 0 || b - t <= 0) return

            val now = android.os.SystemClock.uptimeMillis()
            val cached = svCacheBmp
            val usable = cached != null && !cached.isRecycled && svCacheW == r - l && svCacheH == b - t
            if (usable) {
                // 投影区在录制画布上的位置：SurfaceView 原点 + 投影偏移
                val dx = baseTx + (sLoc[0] - hostLoc[0] + l).toFloat()
                val dy = baseTy + (sLoc[1] - hostLoc[1] + t).toFloat()
                val save = canvas.save()
                canvas.translate(dx, dy)
                canvas.drawBitmap(cached!!, 0f, 0f, surfacePaint)
                canvas.restoreToCount(save)
                // 缓存过期 → 异步刷一帧（不等待，下一帧生效）
                if (now - svCacheAt > COPY_INTERVAL_MS && now - svLastRequestAt > COPY_INTERVAL_MS && !svCopyInFlight) {
                    requestSurfaceCopy(sv, l, t, r, b, glassView)
                }
            } else {
                // 尺寸变化或首帧：回收旧缓存，异步拷（本帧无视频，下一帧到位）
                if (cached != null && !cached.isRecycled && (svCacheW != r - l || svCacheH != b - t)) {
                    svCacheBmp = null
                    cached.recycle()
                }
                if (now - svLastRequestAt > COPY_INTERVAL_MS && !svCopyInFlight) requestSurfaceCopy(sv, l, t, r, b, glassView)
            }
        } catch (_: Throwable) {}
    }

    private fun requestSurfaceCopy(
        sv: android.view.SurfaceView,
        l: Int, t: Int, r: Int, b: Int,
        glassView: View
    ) {
        svCopyInFlight = true
        svLastRequestAt = android.os.SystemClock.uptimeMillis()
        try {
            val bmp = Bitmap.createBitmap(r - l, b - t, Bitmap.Config.ARGB_8888)
            android.view.PixelCopy.request(
                ...
                { res ->
                    try {
                        if (res == android.view.PixelCopy.SUCCESS && !bmp.isRecycled) {
                            val old = svCacheBmp
                            svCacheBmp = bmp
                            svCacheAt = android.os.SystemClock.uptimeMillis()
                            svCacheW = bmp.width
                            svCacheH = bmp.height
                            old?.recycle()
                            // 新帧到位：让玻璃下一帧用上（回调在 copy 线程）。
                            // 玻璃已脱离窗口（菜单关了）就不再触发，防止无主重绘
                            if (glassView.isAttachedToWindow) glassView.postInvalidateOnAnimation()
                        } else {
                            bmp.recycle()
                        }
                    } catch (_: Throwable) {
                    } finally {
                        svCopyInFlight = false
                    }
                },
                copyHandler
            )
    ...
    private fun collectSurfaceViews(v: View, glass: View, out: ArrayList<android.view.SurfaceView>) {
        if (v === glass) return
        if (v is android.view.SurfaceView) { out.add(v); return }
        if (v is ViewGroup) {
            for (i in 0 until v.childCount) {
                collectSurfaceViews(v.getChildAt(i) ?: continue, glass, out)
            }
        }
    }

    // PixelCopy 回调线程：绝不能用主线程 Handler——draw 在主线程执行，
    // 回调若 post 到主线程而主线程阻塞在 latch 上就是死锁超时
    private val copyThread = android.os.HandlerThread("GlassPixelCopy").apply { start() }
    private val copyHandler = android.os.Handler(copyThread.looper)
